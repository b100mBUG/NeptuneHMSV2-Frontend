...
from database.actions.lab_tests import (
    add_lab_tests, edit_lab_test, delete_lab_test,
    search_lab_tests, fetch_lab_tests
)
...
def fetch_tests(intent="all", sort_term="all", sort_dir="desc", search_term="ss", callback=None):
    ...
    Thread(target=fetch_and_return_offline_tests, args=(intent, sort_term, sort_dir, search_term, callback), daemon=True).start()
...
def fetch_and_return_offline_tests(intent, sort_term, sort_dir, search_term, callback):
    try:
        if intent == "all":
            db_result = asyncio.run(fetch_lab_tests(store.get('hospital')['hsp_id'], sort_term, sort_dir))
        elif intent == "search":
            db_result = asyncio.run(search_lab_tests(store.get('hospital')['hsp_id'], search_term))
        ...
        data = [
            {
                ...
                "date_added": f"{test.date_added}"
            } for test in db_result
        ]
    except Exception:
        data = None
...
def add_test(data):
    ...
    #if response.status_code != 200:
        #show_snack("Failed to add test")
        #return
    #show_snack("Test added successfully. You can refresh the page to view them")
    try:
        asyncio.run(add_lab_tests(store.get('hospital')['hsp_id'], data))
        show_snack("Test added successfuly")
...
def edit_test(data, test_id):
    ...
    #print(data)
    #if response.status_code != 200:
        #show_snack("Failed to edit test")
        #return
   # show_snack("Test edited successfully. You can refresh the page to view them")
    try:
        asyncio.run(edit_lab_test(store.get('hospital')['hsp_id'], data))
        show_snack("Test edited successfully")
    except Exception as e:
        show_snack("An unexpected error occurred. Please try again")
        return
...
def delete_test(test_id):
    ...
    #if response.status_code != 200:
        #show_snack("Failed to delete test")
        #return
    #show_snack("Test deleted successfully. You can refresh the page to view them")
    try:
        asyncio(delete_lab_test(store.get('hospital')['hsp_id'], test_id))
        show_snack("Test deleted successfully.")
    except Exception as e:
        show_snack("An unexpected error occurred. Please try again.")
        return
