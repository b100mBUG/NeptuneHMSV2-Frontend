...
def fetch_tests(intent="all", sort_term="all", sort_dir="desc", search_term="ss", callback=None):
    Thread(target=fetch_and_return_tests, args=(intent, sort_term, sort_dir, search_term, callback), daemon=True).start()

def fetch_and_return_tests(intent, sort_term, sort_dir, search_term, callback):
    url = ""
    if intent == "search":
